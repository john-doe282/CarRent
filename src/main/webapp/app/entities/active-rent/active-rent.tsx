...
import { getEntities } from './active-rent.reducer';
...
export const ActiveRent = (props: IActiveRentProps) => {
  ...
  const { activeRentList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="active-rent-heading">
        Active Rents
      ...
      <div className="table-responsive">
        {activeRentList && activeRentList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {activeRentList.map((activeRent, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${activeRent.id}`} color="link" size="sm">
                      {activeRent.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${activeRent.id}`} color="info" size="sm">
          ...
          !loading && <div className="alert alert-warning">No Active Rents found</div>
        )}
      </div>
      {props.totalItems ? (
        <div className={activeRentList && activeRentList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ activeRent }: IRootState) => ({
  activeRentList: activeRent.entities,
  loading: activeRent.loading,
  totalItems: activeRent.totalItems,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(ActiveRent);
