...
import { getEntity, deleteEntity } from './active-rent.reducer';

export interface IActiveRentDeleteDialogProps extends StateProps, DispatchProps, RouteComponentProps<{ id: string }> {}

export const ActiveRentDeleteDialog = (props: IActiveRentDeleteDialogProps) => {
  useEffect(() => {
  ...
  const confirmDelete = () => {
    props.deleteEntity(props.activeRentEntity.id);
  };
...
const mapStateToProps = ({ activeRent }: IRootState) => ({
  ...
  updateSuccess: activeRent.updateSuccess,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(ActiveRentDeleteDialog);
